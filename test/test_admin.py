...
def get_node_ip(node_name):
    node_ip = subprocess.run(
        ["sshpass", "-p", "stardogpw", "ssh", "-o", "StrictHostKeyChecking=no",
         "ssh://stardog@" + node_name + ":2222", "--",
         "hostname", "-I"],
...
def wait_standby_node_to_join(admin):
    standby_node_ip = get_node_ip("pystardog_standby_node").decode("utf-8").strip() + ':5820'
    retries = 0
    while True:
...
def test_cluster_standby(cluster_standby_node_conn_string):
    ...
    with stardog.admin.Admin(**cluster_standby_node_conn_string) as admin_standby:
        ...
        standby_node_info = get_node_ip("pystardog_standby_node").decode('utf-8').strip() + ':5820'
...
def test_backup_all(admin):
    ...
    default_backup = subprocess.run(
        ["sshpass", "-p", "stardogpw", "ssh", "-o", "StrictHostKeyChecking=no", "ssh://stardog@pystardog_stardog_1:2222", "--",
